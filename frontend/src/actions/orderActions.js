...
// --- IMPORTADO ---
import { BASE_URL, ORDERS_URL } from '../constants/apiConstants.js';
...
export const payOrder = (orderId, paymentResult) => async (dispatch, getState) => {
  try {
    ...
    // --- MODIFICADO ---
    const { data } = await axios.put(`${BASE_URL}${ORDERS_URL}/${orderId}/pay`, paymentResult, config);
...
export const deliverOrder = (order) => async (dispatch, getState) => {
    try {
        ...
        const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
        // --- MODIFICADO ---
        const { data } = await axios.put(`${BASE_URL}${ORDERS_URL}/${order._id}/deliver`, {}, config);
...
export const getOrderSummary = () => async (dispatch, getState) => {
    try {
        ...
        const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
        // --- MODIFICADO ---
        const { data } = await axios.get(`${BASE_URL}${ORDERS_URL}/summary`, config);
